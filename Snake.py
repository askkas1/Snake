...
class Field():
    ...
    def get_empty_field(self):
        ...
        while counter != 0:
            counter -=1
            x, y = (randint(0, self.count_blocks_x-1), randint(0, self.count_blocks_y-1))
...
class Snake():
    ...
    def generate_snake(self, field):
        ...
        while True:
            self.snake_xy = []
            x, y = (randint(1, field.count_blocks_x - 1), randint(2, field.count_blocks_y - 1))
            for l in range(field.snake_length):
                self.snake_xy.append((x, y))
                y += 1
            for x, y in self.snake_xy:
                flag_not_empty = self.check_cell_in_field(field, x, y)
    ...
    def move(self, field, direction):
        ...
        if move_result in [0, 1]:
            field.cell_fill_color(self.snake_xy[-1][0],self.snake_xy[-1][1], field.get_color(self.snake_xy[-1][0], self.snake_xy[-1][1],))
    ...
    def dqn_get_field(self,field):
        x,y=self.snake_xy[0]
        return [[self.get_reward(self.check_cell_in_field(field,xx,yy)) for xx in range(x-self.dqn_vision_cnt,x+self.dqn_vision_cnt+1)]for yy in range(y-self.dqn_vision_cnt,y+self.dqn_vision_cnt+1) ]
